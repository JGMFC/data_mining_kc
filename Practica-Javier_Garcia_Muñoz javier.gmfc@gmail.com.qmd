...
```{r}
df_madrid_clean <- na.omit(df_madrid[,c('Accommodates'
                                            ,'Bathrooms'
                                            ,'Bedrooms'
                                            ,'Beds'
                                            ,'Price'
...
#Aplicamos PCA
pca <- prcomp(df_madrid_clean, center=TRUE, scale.=TRUE)
str(pca)
```
...
distance <- dist(rbind(piso_test_pca, predict(pca, newdata=df_madrid_clean)))
df_madrid_clean$distance <- distancia[1:nrow(df_madrid_clean)]
top_5 <- df_madrid_clean[1:5,]
top_5
```
